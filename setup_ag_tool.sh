...
# --- Configuration ---
INSTALL_RICH=false # Default: DO NOT install rich-cli/less
UNINSTALL=false
INSTALL_FLAG_PASSED=false # Track if --install-rich or --no-rich was passed

# --- Argument Parsing ---
# Use getopt for better flag handling
TEMP=$(getopt -o '' --long install-rich,no-rich,uninstall,help -n "$0" -- "$@")
...
while true; do
  case "$1" in
    '--install-rich' ) INSTALL_RICH=true; INSTALL_FLAG_PASSED=true; shift ;;
    '--no-rich' ) INSTALL_RICH=false; INSTALL_FLAG_PASSED=true; shift ;; # Explicit skip
    '--uninstall' ) UNINSTALL=true; shift ;;
    '--help' )
      echo "Usage: $0 [--install-rich | --no-rich] [--uninstall] [--help]"
      echo "  Installs or uninstalls the 'ag' AI assistant tool."
      echo ""
      echo "  Installation Options (Default: --no-rich):"
      echo "    --install-rich   Install base dependencies AND optional Markdown tools (pipx, rich-cli, less)."
      echo "    --no-rich        Install only base dependencies (fish, jq, curl)."
      echo ""
      echo "  Other Options:"
      echo "    --uninstall      Attempt to remove the 'ag' function and related files."
...
SCRIPT_LANG="en" # Default
if [[ "$UNINSTALL" == false ]]; then
    while [[ "$SCRIPT_LANG" != "en" && "$SCRIPT_LANG" != "zh" ]]; do
        read -p "Please choose language for setup instructions (请输入安装提示语言) [en/zh]: " SCRIPT_LANG
...
# --- Message Definitions ---
# (Messages remain the same as the previous multilingual version)
# --- English Messages ---
...
MSG_INFO_RICH_FOUND_EN="'rich' command already found. Assuming it's functional."
...
MSG_INFO_SKIPPING_RICH_EN="Skipping installation of optional Markdown tools (rich-cli, less) by default or request."
...
MSG_INFO_RICH_DISABLED_NOTE_EN="Note: Optional Markdown rendering tools were not installed. Features requiring them (-m, -o viewing) will not work unless you install rich-cli and less manually or re-run setup with --install-rich."
MSG_INFO_UNINSTALL_START_EN="Starting uninstallation process for 'ag'..."
MSG_INFO_REMOVE_AG_FILE_EN="Removing 'ag.fish' function file:"
MSG_ERR_REMOVE_AG_FILE_FAILED_EN="Failed to remove 'ag.fish' file. Please remove it manually."
MSG_INFO_AG_FILE_REMOVED_EN="'ag.fish' removed."
MSG_INFO_AG_FILE_NOT_FOUND_EN="'ag.fish' function file not found."
MSG_PROMPT_REMOVE_CONTEXT_EN="Do you want to remove the context directory (\$1)? [y/N]: "
MSG_INFO_REMOVE_CONTEXT_DIR_EN="Removing context directory:"
MSG_ERR_REMOVE_CONTEXT_DIR_FAILED_EN="Failed to remove context directory. Please remove it manually."
MSG_INFO_CONTEXT_DIR_REMOVED_EN="Context directory removed."
MSG_INFO_CONTEXT_DIR_KEPT_EN="Keeping context directory."
MSG_PROMPT_UNINSTALL_RICH_EN="Do you want to attempt uninstalling 'rich-cli' using pipx? [y/N]: "
MSG_INFO_UNINSTALL_RICH_EN="Attempting to uninstall 'rich-cli' via pipx..."
MSG_INFO_RICH_UNINSTALLED_EN="'rich-cli' uninstalled via pipx."
MSG_ERR_RICH_UNINSTALL_FAILED_EN="Failed to uninstall 'rich-cli' via pipx. You may need to do it manually ('pipx uninstall rich-cli')."
MSG_INFO_RICH_NOT_INSTALLED_PIPX_EN="'rich-cli' not found in pipx managed packages."
MSG_INFO_UNINSTALL_COMPLETE_EN="Uninstallation complete. Base packages (fish, jq, curl, pipx, less) were not removed."
...
MSG_INFO_RICH_FOUND_ZH="已找到 'rich' 命令。假定其可用。"
...
MSG_INFO_SKIPPING_RICH_ZH="默认或根据请求跳过安装可选的 Markdown 工具 (rich-cli, less)。"
...
MSG_INFO_RICH_DISABLED_NOTE_ZH="注意：未安装可选的 Markdown 渲染工具。需要这些工具的功能 (-m, -o 查看) 将无法工作，除非您手动安装 rich-cli 和 less 或使用 --install-rich 重新运行安装脚本。"
MSG_INFO_UNINSTALL_START_ZH="开始卸载 'ag' 工具..."
MSG_INFO_REMOVE_AG_FILE_ZH="正在删除 'ag.fish' 函数文件:"
MSG_ERR_REMOVE_AG_FILE_FAILED_ZH="删除 'ag.fish' 文件失败。请手动删除。"
MSG_INFO_AG_FILE_REMOVED_ZH="'ag.fish' 已删除。"
MSG_INFO_AG_FILE_NOT_FOUND_ZH="未找到 'ag.fish' 函数文件。"
MSG_PROMPT_REMOVE_CONTEXT_ZH="您想删除上下文目录 (\$1) 吗? [y/N]: "
MSG_INFO_REMOVE_CONTEXT_DIR_ZH="正在删除上下文目录:"
MSG_ERR_REMOVE_CONTEXT_DIR_FAILED_ZH="删除上下文目录失败。请手动删除。"
MSG_INFO_CONTEXT_DIR_REMOVED_ZH="上下文目录已删除。"
MSG_INFO_CONTEXT_DIR_KEPT_ZH="保留上下文目录。"
MSG_PROMPT_UNINSTALL_RICH_ZH="您想尝试使用 pipx 卸载 'rich-cli' 吗? [y/N]: "
MSG_INFO_UNINSTALL_RICH_ZH="正在尝试通过 pipx 卸载 'rich-cli'..."
MSG_INFO_RICH_UNINSTALLED_ZH="已通过 pipx 卸载 'rich-cli'。"
MSG_ERR_RICH_UNINSTALL_FAILED_ZH="通过 pipx 卸载 'rich-cli' 失败。您可能需要手动卸载 ('pipx uninstall rich-cli')。"
MSG_INFO_RICH_NOT_INSTALLED_PIPX_ZH="在 pipx 管理的包中未找到 'rich-cli'。"
MSG_INFO_UNINSTALL_COMPLETE_ZH="卸载完成。基础软件包 (fish, jq, curl, pipx, less) 未被移除。"
...
CONTEXT_DIR_VARNAME="CONTEXT_DIR" # Variable name used inside ag.fish
DEFAULT_CONTEXT_DIR="$HOME/.local/share/ag_contexts" # Default value if not found in script

# --- Uninstall Logic ---
if [[ "$UNINSTALL" == true ]]; then
    SCRIPT_LANG="en" # Use English for uninstall prompts for simplicity
    print_info INFO_UNINSTALL_START

    # Remove ag.fish
    if [[ -f "$AG_SCRIPT_PATH" ]]; then
        print_info INFO_REMOVE_AG_FILE "$AG_SCRIPT_PATH"
        if rm "$AG_SCRIPT_PATH"; then
            print_info INFO_AG_FILE_REMOVED
        else
            print_warning ERR_REMOVE_AG_FILE_FAILED
        fi
    else
        print_info INFO_AG_FILE_NOT_FOUND
    fi

    # Ask to remove context directory
    CONTEXT_DIR_ACTUAL=$(grep -oP "set -l $CONTEXT_DIR_VARNAME \"\K[^\"]+" "$AG_SCRIPT_PATH" 2>/dev/null || echo "$DEFAULT_CONTEXT_DIR")
    # Expand potential ~
    CONTEXT_DIR_ACTUAL="${CONTEXT_DIR_ACTUAL/#\~/$HOME}"

    if [[ -d "$CONTEXT_DIR_ACTUAL" ]]; then
        read -p "$(print_message PROMPT_REMOVE_CONTEXT "$CONTEXT_DIR_ACTUAL")" -n 1 -r REPLY
        echo # Move to new line
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            print_info INFO_REMOVE_CONTEXT_DIR "$CONTEXT_DIR_ACTUAL"
            if rm -rf "$CONTEXT_DIR_ACTUAL"; then
                print_info INFO_CONTEXT_DIR_REMOVED
            else
                print_warning ERR_REMOVE_CONTEXT_DIR_FAILED
            fi
        else
            print_info INFO_CONTEXT_DIR_KEPT
        fi
    fi

    # Ask to uninstall rich-cli
    if command_exists pipx; then
        read -p "$(print_message PROMPT_UNINSTALL_RICH)" -n 1 -r REPLY
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
             print_info INFO_UNINSTALL_RICH
             if pipx list --short | grep -q "rich-cli"; then
                 if pipx uninstall rich-cli; then
                     print_info INFO_RICH_UNINSTALLED
                 else
                     print_warning ERR_RICH_UNINSTALL_FAILED
                 fi
             else
                 print_info INFO_RICH_NOT_INSTALLED_PIPX
             fi
        fi
    fi
...
# --- !!! Embed the ag.fish script PROVIDED BY USER (with syntax issues for Fish < 3) !!! ---
...
function ag --description "ag: 向 OpenRouter 提问，可选多文件上下文、保存响应、Markdown渲染"
    ...
    if set -q _flag_help
        echo "用法: ag [选项] \"<你的问题>\""
        echo ""
        echo "向 OpenRouter API 提问，支持流式纯文本输出、保存响应到文件、"
        echo "或在终端渲染 Markdown。可通过 -s 选择或创建命名上下文。"
        ...
        printf "  %-25s %s\n" " " "  (若同时用 -m, 请求 Markdown; 否则请求纯文本)"
        printf "  %-25s %s\n" "-h, --help" "显示此帮助信息并退出"
        echo ""
        echo "上下文存储目录: $CONTEXT_DIR"
        # --- NOTE: This line uses Bash-style &&/|| which is invalid in Fish ---
        echo "默认输出目录 (\$AG_DEFAULT_OUTPUT_DIR): "(set -q AG_DEFAULT_OUTPUT_DIR && echo $AG_DEFAULT_OUTPUT_DIR || echo 未设置)
        # --- End invalid line ---
        echo ""
        echo "依赖: jq. 可选: rich-cli, less (用于 -m), xdg-utils/open (用于 -o 后打开)"
        echo ""
        echo "示例:"
        echo "  ag \"你好\"                         # 即时对话"
        ...
        echo "  ag -m -o out.md -s projB \"...\" # 使用 projB 上下文, 保存响应并渲染"
        return 0
    end
    ...
    if set -q _flag_output; or set -q _flag_markdown # 如果保存文件或请求 markdown 渲染
        if not command -q rich; echo "错误：需要 'rich-cli'。" >&2; return 1; end
    end
    if set -q _flag_markdown; and not set -q _flag_output # 仅终端渲染时需要 less
        if not command -q less; echo "错误：需要 'less'。" >&2; return 1; end
    end
    set -l open_command "" # 用于 -o 后打开文件
    if test -n "$output_file"; if command -q xdg-open; set open_command xdg-open; else if command -q open; set open_command open; end; end
    ...
    if set -q _flag_select
        ...
        if test -e "$context_file_to_use"; set messages_json_array (cat "$context_file_to_use" 2>/dev/null); if test $status -ne 0; or test -z "$messages_json_array"; echo "警告: 无法读取或上下文文件为空 '$context_file_to_use'. 将创建新上下文。" >&2; set messages_json_array '[]'; else if not echo "$messages_json_array" | jq -e 'type == "array"' > /dev/null 2>&1; echo "警告: 上下文文件 '$context_file_to_use' 内容不是有效的 JSON 数组。将创建新上下文。" >&2; set messages_json_array '[]'; end; end; else; echo "✨ 上下文文件 '$context_file_to_use' 不存在，将创建新的对话。" >&2; end
    end

    # --- 配置 System Prompt ---
    set -l system_prompt_base "根据需要进行适当的换行和分段。回答尽量详细，将我当作小白来解释。"
    set -l system_prompt_format_instruction
    if set -q _flag_output; or set -q _flag_markdown; set system_prompt_format_instruction "请使用 Markdown 格式进行回复（包括代码块、列表、加粗等）。"; else; set system_prompt_format_instruction "请始终使用纯文本格式进行回复,可以使用emoji,但也不宜太多。绝对不要使用任何Markdown标记（如\`*\`、\`#\`、\` \`\`\` \`、\\"-\\"等），因为输出环境是终端。"; end
    ...
    # --- NOTE: The following lines use single-line ifs with '; return', which is invalid in Fish ---
    if test $status -ne 0; echo "错误: 构建系统消息失败。" >&2; return 1; end
    if test -n "$messages_json_array"; and echo "$messages_json_array" | jq -e 'type == "array"' > /dev/null 2>&1; set current_call_messages (echo $current_call_messages $messages_json_array | jq -s '.[0] + .[1]'); if test $status -ne 0; echo "错误: 合并历史消息失败。" >&2; return 1; end; end
    set current_call_messages (echo $current_call_messages | jq --arg content "$user_prompt" '. + [{"role": "user", "content": $content}]'); if test $status -ne 0; echo "错误: 添加用户消息失败。" >&2; return 1; end
    # --- End invalid lines ---
    ...
    # --- NOTE: Single-line if with '; return' is invalid in Fish ---
    ...
    set -l full_response ""; set -l curl_exit_status -1; set -l process_exit_status -1
    begin # pipeline start
        curl --silent --no-buffer -X POST "$api_endpoint" -H "Content-Type: application/json" -H "Authorization: Bearer $OPENROUTER_API_KEY" -d "$json_payload" | while read -l line
            if string match -q "data: *" -- "$line"
                set json_chunk (string sub -s 7 -- "$line"); if test "$json_chunk" = "[DONE]"; break; end
                set api_error (echo "$json_chunk" | jq -r '.error.message // ""'); if test -n "$api_error"; echo "API 错误: $api_error" >&2; continue; end
                set text_chunk (echo "$json_chunk" | jq -r '.choices[0].delta.content // ""'); set jq_status $status
                if test $jq_status -ne 0; echo "警告：解析 JSON 块失败: $json_chunk" >&2
                else; if not set -q _flag_output; and not set -q _flag_markdown; printf '%s' "$text_chunk"; end; set full_response "$full_response$text_chunk"; end
            end
        end # while end
    end # pipeline end
    set curl_exit_status $pipestatus[1]; set process_exit_status $pipestatus[-1]
    ...
    if test -n "$output_file"; and test $curl_exit_status -eq 0; and test $process_exit_status -eq 0; and test -n "$full_response"
        ...
        set -l tmp_file "/tmp/ag_render."(random)".md"; printf '%s' "$full_response" > "$tmp_file"; if test $status -eq 0; rich "$tmp_file" | less -R; set render_status $pipestatus[1]; if test $render_status -ne 0; echo "警告: rich 命令渲染可能出错 (状态码: $render_status)" >&2; end; rm "$tmp_file"; else; echo "错误: 无法创建临时文件进行渲染" >&2; echo "$full_response"; set render_status 1; end
    ...
    if test -n "$selected_context_name"; and test $curl_exit_status -eq 0; and test $process_exit_status -eq 0; and test -n "$full_response"
        set context_file_to_save "$CONTEXT_DIR/$selected_context_name.json"; set -l updated_context_json; set -l jq_status -1
        set updated_context_json (echo $messages_json_array | jq --arg user_msg "$user_prompt" --arg assistant_msg "$full_response" '. + [{"role": "user", "content": $user_msg}, {"role": "assistant", "content": $assistant_msg}]'); set jq_status $status
        if test $jq_status -eq 0; if test "$updated_context_json" != "$messages_json_array"; printf '%s\n' "$updated_context_json" > "$context_file_to_save"; if test $status -ne 0; echo "错误：无法将更新后的上下文写入文件 '$context_file_to_save'" >&2; end; end; else; echo "错误：使用 jq 更新内存中的上下文失败 (状态码: $jq_status)。上下文未保存到文件。" >&2; end
    end

    # --- 最终返回码 ---
    if test $curl_exit_status -ne 0; or test $process_exit_status -ne 0; or test $save_status -ne 0; or test $render_status -ne 0; return 1; end
    return 0
end # <--- 函数定义的唯一结束 end
